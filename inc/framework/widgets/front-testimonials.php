...
class Atframework_Testimonials extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array('classname' => 'atframework_testimonials_widget', 'description' => __( 'Display your testimonials.', 'exago') );
        parent::__construct(false, $name = __('exago FP: Testimonials', 'exago'), $widget_ops);
	...
	<p><?php _e('This widget displays all pages that have the Single Testimonial page template assigned to them.', 'exago'); ?></p>
	<p><em><?php _e('Tip: to rearrange the testimonials order, edit each testimonial page and add a value in Page Attributes > Order', 'exago'); ?></em></p>
	<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'exago'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
	</p>
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of testimonials to show (-1 shows all of them):', 'exago' ); ?></label>
	<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
	<p><label for="<?php echo $this->get_field_id( 'offset' ); ?>"><?php _e( 'Offset (number of testimonials needs to be different than -1 for this option to work):', 'exago' ); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all'); ?>"><?php _e('The URL for your button [In case you want a button below your testimonials block]', 'exago'); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all_text'); ?>"><?php _e('The text for the button [Defaults to <em>See all our testimonials</em> if left empty]', 'exago'); ?></label>
			...
			<?php if ( $title ) echo $before_title . $title . $after_title; ?>
				...
				<div class="testimonial-area clearfix">
					<?php while ( $testimonials->have_posts() ) : $testimonials->the_post(); ?>
						<div class="testimonial exago-3col">
							...
							<div class="testimonial-thumb">
								<?php the_post_thumbnail('exago-testimonial-thumb'); ?>
							</div>
							<?php endif; ?>
				...
				<?php if ($see_all != '') : ?>
					<a href="<?php echo esc_url($see_all); ?>" class="button centered-button">
						...
						<?php else : ?>
							<?php echo __('See all our testimonials', 'exago'); ?>
						<?php endif; ?>
					</a>
				<?php endif; ?>
	<?php
