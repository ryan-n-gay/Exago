...
class Atframework_Posts extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array('classname' => 'atframework_posts_widget', 'description' => __( 'Show the latest news from your blog.', 'exago') );
        parent::__construct(false, $name = __('exago FP: Latest News', 'exago'), $widget_ops);
	...
	<p><label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( 'Enter the slug for your category or leave empty to show posts from all categories.', 'exago' ); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id( 'category' ); ?>" name="<?php echo $this->get_field_name( 'category' ); ?>" type="text" value="<?php echo $category; ?>" size="3" /></p>

    <p><label for="<?php echo $this->get_field_id('see_all_text'); ?>"><?php _e('Add the text for the button here if you want to change the default <em>See all our news</em>', 'exago'); ?></label>
	...
	function widget($args, $instance) {
		...
		$category = isset( $instance['category'] ) ? esc_attr($instance['category']) : '';
		$see_all_text = isset( $instance['see_all_text'] ) ? esc_html($instance['see_all_text']) : __( 'See all our news', 'exago' );
		if ($see_all_text == '') {
			$see_all_text = __( 'See all our news', 'exago' );
		}
...
?>
		...
		<?php while ( $r->have_posts() ) : $r->the_post(); ?>
			<div class="blog-post exago-3col">
